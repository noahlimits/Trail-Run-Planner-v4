...
───────────────────────────────────────────────────────
Complete implementation with corrected session-based WU/CD and Roche logic (no KeyError).
"""
...
st.caption("*No race? The 12-week evergreen block targets trail events ≈ 30–50 km.*")

# Constants & tables
VERT_TARGETS = {"Road/Flat":50,"Flat Trail":150,"Hilly Trail":300,"Mountainous/Skyrace":500}
FUEL_TABLE = pd.DataFrame({"Condition":["< 20 °C","+10 °C","+20 °C"],
                            "CHO (g h⁻¹)":["60–80","70–90","80–100"],
                            "Fluid (ml h⁻¹)":["500–600","550–700","600–800"]})
GLOSSARY = {
    "Roche Treadmill Uphill": "Set treadmill ≈6.5 km/h; raise incline until HR≈VT1, then increase speed.",
    "Hill Beast": "10/8/6/4/2 min uphill @ threshold; jog equal recoveries.",
    "Aggressive Downhill": "15 min VT1 uphill + 6–8×90 s downhill (−8–12%) + 10 min CD.",
    "Plyometrics": "Box jumps×6, bounds 30m, skater bounds×10ea, single‑leg hops×10ea (Base:1 set; Speed:2).",
    "Heavy Lifts": "Back‑Squat/Bulgarian Split Squat, Deadlift, RDL, Pull‑ups — 3‑4×5 @80–85%1RM.",
    "VT1 Test": "Talk‑test: first speech change = VT1; confirm with HR drift <3% over two 5‑min blocks.",
}
DISTANCE_SUGGEST = {"10 km":"3–5","21 km":"5–7","42 km":"6–10",
...
with st.sidebar:
    ...
    c2.markdown("<span title='VT1 protocol in Variables & Guidance → Glossary'>ℹ️</span>", unsafe_allow_html=True)
    ...
    if hrs_hi < hrs_lo: hrs_hi = hrs_lo
    weekly_hours = f"{hrs_lo}-{hrs_hi}" if hrs_lo != hrs_hi else str(hrs_lo)

    def rec_key(km:int)->str:
        if km<=12: return "10 km"
        if km<=30: return "21 km"
        if km<=45: return "42 km"
        if km<=60: return "50 km"
        if km<=85: return "70 km"
        return "100 km"
...
# Helper logic based on Session

def _needs_wucd(session: str) -> bool:
    low = session.lower()
    return not any(k in low for k in ("easy", "recovery", "rest"))


def _split_wucd(session: str) -> Tuple[str,str]:
    return ("10 min EZ","10 min EZ") if _needs_wucd(session) else ("","")
...
def _insert_downhill(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        if r["Day"]=="Sunday" and r["Week"]%3==0:
            ...
            d["Description"] = "15 min VT1 uphill + 6–8×90 s downhill (−8–12%) walk-back + 10 min CD"
            d["Duration"] = "60 min"
            out.append(d)
    ...
    for wk,val in weekly.items():
        if val < 0.9 * target:
            mask = (
                (df["Week"]==wk)
                & (df["Shift?"]=="Shift")
                & df["Session"].str.lower().str.contains("easy")
            )
            cnt=0
...
def _schedule_hwi(df: pd.DataFrame, is_race: bool, mask: pd.Series) -> pd.DataFrame:
    notes: List[str] = []
    for i,r in df.iterrows():
        if mask.iloc[i]: notes.append("")
        elif is_race:
            if i<14: notes.append("HWI 30 min @40°C (load)")
            elif (df["Date"].max()-r["Date"]).days<10 and r["Day"] in {"Tuesday","Friday"}:
                notes.append("HWI 20 min @40°C (taper)")
            else: notes.append("HWI 25 min @40°C (maint)")
        else:
            if i<10: notes.append("HWI 30 min @40°C (load)")
            elif r["Day"] in {"Monday","Wednesday","Friday"}: notes.append("HWI 25 min @40°C (maint)")
            else: notes.append("")
...
if run_btn:
    ...
    comp_df[["WU","CD"]] = pd.DataFrame(
        comp_df["Session"].apply(_split_wucd).tolist(), index=comp_df.index
    )
    ...
    if add_race and not race_df.empty:
        race_df[["WU","CD"]] = pd.DataFrame(
            race_df["Session"].apply(_split_wucd).tolist(), index=race_df.index
        )
        ...
        race_df["Block Focus"] = race_df["Week"].apply(
            lambda w: "Base/Economy" if w<=2 else ("Threshold/VO₂" if w<=4 else ("Speed-Endurance" if w<=7 else "Taper"))
        )

    # Tabs
    tabs = st.tabs(["Evergreen Plan","Race Plan","Variables & Guidance","Info & References"])
    with tabs[0]: st.dataframe(comp_df, use_container_width=True, height=800)
    with tabs[1]:
        if add_race and not race_df.empty: st.dataframe(race_df, use_container_width=True, height=800)
        else: st.info("Race build not generated.")
    with tabs[2]:
        st.subheader("Weekly Hours Guidance"); st.table(pd.DataFrame({"Distance":list(DISTANCE_SUGGEST.keys()),"Hours":list(DISTANCE_SUGGEST.values())}))
        st.subheader("Fuel & Hydration Guidance"); st.table(FUEL_TABLE)
        st.subheader("Exercise Glossary");
        for name,desc in GLOSSARY.items(): st.markdown(f"**{name}** — {desc}")
    with tabs[3]:
        st.subheader("Block & Taper Rationale"); st.markdown("*Base/Economy → Threshold/VO₂ → Speed-Endurance → 2-week taper.*")
        st.subheader("Shift-Cycle Explanation"); st.markdown("Aligns long runs to Off days (48/96 schedule).")
        st.subheader("Heat-Training Background"); st.markdown("Loading & maintenance as per Patterson 2021 & Casadio 2024; skip shift days.")
        st.subheader("References"); st.markdown("""
...
""")
    ...
    if add_race and not race_df.empty: st.download_button("Download Race CSV", race_df.to_csv(index=False).encode(), "race.csv", mime="text/csv")
